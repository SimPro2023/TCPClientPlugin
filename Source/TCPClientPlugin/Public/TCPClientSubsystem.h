...
/**
 * @brief Globally accessible class for managing TCP connections.
 */
UCLASS()
...
public:
	...
	/** @brief Attempt to connect to the server. Returns instance of session. */
	UTCPSessionBase* ConnectSession(TSubclassOf<UTCPSessionBase> session);

	/**
	 * @brief Attempt to connect to the server. Blueprint version.
	 * @param session class
	 * @param connectDelegate delegate for connection
	 * @param disconnectDelegate delegate for disconnection
	...
	/** @brief Terminate the connection by session name */
	UFUNCTION(BlueprintCallable, Category = "TCPClientSubsystem")
	...
	/** 
	 * @brief Fetch a delayed session when connection status is uncertain
	 * @param sessionName
	 * @param connectDelegate
	 */
	UFUNCTION(BlueprintCallable, Category = "TCPClientSubsystem")
	void GetSessionLazy(const FString& sessionName, const FConnectedSessionDelegate& connectDelegate);

#pragma region FTickableGameObject override
...
public:
	/** Receives all connection event messages. Filter by Session name. */
	UPROPERTY(BlueprintAssignable, Category = "TCPSession")
	FConnectedSessionMulticastDelegate OnConnected;

	/** Receives all disconnection event messages. Filter by Session name. */
	UPROPERTY(BlueprintAssignable, Category = "TCPSession")
